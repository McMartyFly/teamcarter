...
class Phoen_Import_Data_Main {
	...
	function all_setting_data( $options ){
	 ...
	 function phoen_data_files( $args = array() ) {
        ...
        $xml_id = $this->cloud_data_file( "http://democontentphoeniixx.com/wp-content/uploads/2018/04/$prefix_name-demo.xml",  $xml_file_name );
		
        if ( $xml_id ) {
            $xml_file = get_attached_file( $xml_id );
        }

	   $config_id = $this->cloud_data_file( "http://democontentphoeniixx.com/wp-content/uploads/2018/04/$prefix_name-config.json",  $config_file_name );
